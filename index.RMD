...
Recall the simulation [parameter](#36) $\alpha = 5$ 
```{r}
mod_jm <- jointModel(lmeObject = mod_t, survObject = mod_cox, timeVar = "t_obs")
jmmod_sum <- summary(mod_jm)
DT::datatable(jmmod_sum$`CoefTable-Event`,options = list(pageLength = 5)) %>% 
  DT::formatRound(columns=colnames(jmmod_sum$`CoefTable-Event`), digits=3)
```
